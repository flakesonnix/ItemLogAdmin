  ...
  outputs = { self, nixpkgs, flake-utils }:
    flake-utils.lib.eachSystem [ "x86_64-linux" "aarch64-linux" "aarch64-darwin" ] (system:
      let pkgs = import nixpkgs { inherit system; }; jdk = pkgs.jdk21; gradle = pkgs.gradle_8; in {
        devShells.default = pkgs.mkShell { buildInputs = [ jdk gradle pkgs.git pkgs.nixfmt ]; shellHook = '' export JAVA_HOME=${jdk}; echo "ItemLogAdmin — java $(java -version 2>&1 | head -n1)" ''; };
