...
class QueryServiceTest {
    ...
    fun `getEvents delegates to repo`() {
        ...
        every { repo.findEvents(any(), any(), any(), any(), any(), any(), any()) } returns listOf(view)
        every { repo.countEvents(any(), any(), any(), any(), any()) } returns 1L
        val service = QueryService(repo)
        val (list, total) = service.getEvents(UUID.randomUUID(), "PICKUP", 0, 45)
        assertEquals(1, list.size)
        assertEquals(1L, total)
        verify { repo.findEvents(any(), "PICKUP", null, null, null, 45, 0) }
        verify { repo.countEvents(any(), "PICKUP", null, null, null) }
    }

    @Test
    fun `getEvents pagination offset`() {
        val repo = mockk<ItemLogQueryRepository>(relaxed = true)
        every { repo.findEvents(any(), any(), any(), any(), any(), any(), any()) } returns emptyList()
        every { repo.countEvents(any(), any(), any(), any(), any()) } returns 100L
        val service = QueryService(repo)
        val (list, total) = service.getEvents(null, null, 2, 45)
        assertEquals(0, list.size)
        assertEquals(100L, total)
        verify { repo.findEvents(null, null, null, null, null, 45, 90) } // page 2 * 45 = 90 offset
    }
    ...
    fun `getEvents with null filters`() {
        val repo = mockk<ItemLogQueryRepository>(relaxed = true)
        every { repo.findEvents(any(), any(), any(), any(), any(), any(), any()) } returns emptyList()
        every { repo.countEvents(any(), any(), any(), any(), any()) } returns 0L
        val service = QueryService(repo)
